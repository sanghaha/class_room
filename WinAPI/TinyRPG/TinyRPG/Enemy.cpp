...
Enemy::Enemy(const MonsterData* data, Vector pos) : Super(pos)
{
...

	{
		...
		_animInfo[AnimType::A_IDLE][DirType::DIR_LEFT].flipX = true;
	}
	{
		...
		_animInfo[AnimType::A_MOVE][DirType::DIR_LEFT].flipX = true;
	}
	{
		_animInfo[AnimType::A_ATTACK][DirType::DIR_RIGHT] = AnimInfo{ 0, 2, 6, 1, false, 0.6f };
		_animInfo[AnimType::A_ATTACK][DirType::DIR_LEFT] = AnimInfo{ 0, 2, 6, 1, false, 0.6f, true };
	...
	// Sprite 생성
	_sprite = CreateSpriteComponent(data->_spriteName);
	SetAnimInfo(_animInfo[AnimType::A_IDLE][DirType::DIR_RIGHT]);
}
