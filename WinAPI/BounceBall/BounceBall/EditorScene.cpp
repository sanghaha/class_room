...
void EditorScene::Render(ID2D1RenderTarget* renderTarget)
{
	...
	{
		D2D1_POINT_2F start = D2D1::Point2F((float)0, (float)y);
		D2D1_POINT_2F end = D2D1::Point2F((float)GWinSizeX, (float)y);
	...
	{
		D2D1_POINT_2F start = D2D1::Point2F((float)x, (float)0);
		D2D1_POINT_2F end = D2D1::Point2F((float)x, (float)GWinSizeY);
		renderTarget->DrawLine(start, end, brush, 1.0f);
	}
