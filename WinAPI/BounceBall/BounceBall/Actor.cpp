...
void SpriteActor::Render(ID2D1RenderTarget* renderTarget)
{
	...
	if (debug)
	{
		...
		{
			renderTarget->DrawLine(line[0], line[1], brushRed, 2.0f);
			renderTarget->DrawLine(line[1], line[2], brushRed, 2.0f);
			renderTarget->DrawLine(line[2], line[3], brushRed, 2.0f);
			renderTarget->DrawLine(line[3], line[0], brushRed, 2.0f);
		}
	}
}
