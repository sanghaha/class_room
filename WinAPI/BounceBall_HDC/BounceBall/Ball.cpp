...
void Ball::applyPhysics_Test(float deltaTime)
{
    GameScene* scene = dynamic_cast<GameScene*>(Game::GetInstance()->GetScene());
    if (nullptr == scene)
        return;
    ...
    // 가속도 및 속도 갱신 (중력 적용)
    acceleration += gravityVec * gravityScale;
    velocity += acceleration * deltaTime;
    if (velocity.y > maxFallSpeed)
        velocity.y = maxFallSpeed;
    velocity.x *= expf(-xFrictionExp * deltaTime); // 수평 공기저항 (수직은 중력이 유지)
    ...
    // 충돌 체크
    if (scene->CheckCollision_ClosestPoint(this, newPos, BALL_SIZE * 0.5f, normal, hitPos))
    {
        // 1. 위치 보정 (겹침 방지)
        newPos = hitPos;

        // 2. 물리적 반사 벡터 계산 (입사각과 반사각을 고려한 자연스러운 방향)
        // reflect = v - 2 * (v · n) * n
        Vector reflect = velocity - (normal * 2.0f * velocity.Dot(normal));
        if (normal.y < 0.f)                  // 바닥: 수직 속도 고정
            reflect.y = -bounceSpeedY * sqrtf(gravityScale);
        if (std::abs(normal.x) > 0.5f)       // 수직 벽: 수평 속도 강제 고정
            reflect.x = bounceSpeedX * normal.x;
        velocity = reflect;
...
void Ball::Render(HDC renderTarget)
{
    ...
    auto DrawArrow = [&](Vector start, Vector dir, float length, COLORREF color)
    {
        if (dir.LengthSquared() < 0.0001f) return;

        Vector normalDir = dir.GetNormalize();
        Vector end = start + normalDir * length;

        HPEN myPen = CreatePen(PS_SOLID, 2, color);
        HPEN oldPen = (HPEN)SelectObject(renderTarget, myPen);

        MoveToEx(renderTarget, (int)start.x, (int)start.y, nullptr);
        LineTo(renderTarget, (int)end.x, (int)end.y);

        // Arrow head
        Vector arrowHead1 = normalDir.Rotate(150) * 10;
        Vector arrowHead2 = normalDir.Rotate(-150) * 10;

        MoveToEx(renderTarget, (int)end.x, (int)end.y, nullptr);
        LineTo(renderTarget, (int)(end.x + arrowHead1.x), (int)(end.y + arrowHead1.y));

        MoveToEx(renderTarget, (int)end.x, (int)end.y, nullptr);
        LineTo(renderTarget, (int)(end.x + arrowHead2.x), (int)(end.y + arrowHead2.y));
    ...
    // Debug Normal (Cyan)
    DrawArrow(_debug_newPos, _debug_normal, 50.f, RGB(0, 255, 255));

    // Debug Reflect (Magenta)
    DrawArrow(_debug_newPos, _debug_reflect, 50.f, RGB(255, 0, 255));

    // Debug Point (Yellow)
    {
        HBRUSH myBrush = CreateSolidBrush(RGB(255, 255, 0));
        HBRUSH oldBrush = (HBRUSH)SelectObject(renderTarget, myBrush);
        Ellipse(renderTarget, (int)_debug_newPos.x - 3, (int)_debug_newPos.y - 3, (int)_debug_newPos.x + 3, (int)_debug_newPos.y + 3);
        SelectObject(renderTarget, oldBrush);
...
{
    if (other->GetActorType() == ActorType::AT_STAR)
    {
        GameScene* gameScene = dynamic_cast<GameScene*>(Game::GetInstance()->GetScene());
