...
bool Scene::CanMoveCell(const Cell& nextCell)
{
	const GridInfo& gridInfo = Game::GetScene()->GetGridInfo(nextCell);
	for (Actor* a : gridInfo._actors)
	{
		if (a->GetActorType() == ActorType::AT_BLOCK)
			return false;
	}
	return true;
}
