...
void TileMapMain::Init(HWND hwnd, std::wstring path)
{
	...
	// 그리드 초기화
	_layer.mainGrid.resize(GridWidth * GridHeight, -1);
}
...
void TileMapMain::Update()
{
	...
	{
	// 선택된 타일을 그리드에 배치
	_layer.mainGrid[y * GridWidth + x] = ToolManager::GetInstance()->GetSelectedTileIndex();
	}
	}
	...
	{
	// 선택된 타일을 그리드에서 제거
	_layer.mainGrid[y * GridWidth + x] = -1;
	}
	}
...
for (int y = 0; y < GridHeight; y++) {
	for (int x = 0; x < GridWidth; x++) {
	int tileIndex = _layer.mainGrid[y * GridWidth + x];
	if (tileIndex >= 0)
		DrawTileOnGrid(hdc, tileIndex, x, y);
	}
}
}

void TileMapMain::DrawTileOnGrid(HDC hdc, int tileIndex, int gridX, int gridY)
{
// 선택한 타일을 그리드에 그리기
...
if (GetOpenFileName(&ofn)) {
	...
	if (file.is_open()) {
		...
		// 그리드 데이터 초기화
		_layer.mainGrid.assign(GridWidth * GridHeight, -1);
		...
		if (line.empty() == false)
		{
			...
			if (x >= 0 && x < GridWidth && y >= 0 && y < GridHeight) {
				_layer.mainGrid[y * GridWidth + x] = tileIndex;
			}
			}
		}
		}
